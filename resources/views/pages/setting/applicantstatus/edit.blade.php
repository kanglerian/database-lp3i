<x-app-layout>
            ...
            <ol class="inline-flex items-center space-x-2 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('applicantstatus.index') }}"
                            class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-gray-800">
                            Master Status Aplikan
                        </a>
                    </div>
                </li>
    ...
    <main class="max-w-7xl w-full mx-auto py-10">
        ...
        @endif
        <section>
            <form action="{{ route('applicantstatus.update', $applicant_status->id) }}" method="POST" class="max-w-sm">
                @csrf
                @method('PATCH')
                <div class="mb-5">
                    <label for="name" class="block mb-2 text-sm font-medium text-gray-900">Status</label>
                    <input type="text" id="name" name="name" value="{{ $applicant_status->name }}"
                        ...
                        placeholder="Status aplikan" required />
                </div>
                <button type="submit"
