<x-app-layout>
    ...
    <main class="max-w-7xl mx-auto px-5 md:p-0 space-y-5">
        @if (session('message'))
            <div id="alert" class="flex items-center p-4 mb-4 bg-emerald-400 text-white rounded-2xl" role="alert">
        ...
        @endif
        @if (session('error'))
            <div id="alert" class="flex items-center p-4 mb-4 bg-red-500 text-white rounded-2xl" role="alert">
        ...
        <section class="flex flex-col md:flex-row items-center justify-between gap-3 md:gap-0">
            <a href="{{ route('user.create') }}" class="bg-lp3i-100 hover:bg-lp3i-200 px-5 py-2.5 text-sm rounded-xl text-white">
                ...
                <div class="relative overflow-x-auto border rounded-3xl">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                        <thead class="text-xs text-gray-700 uppercase">
                            <tr>
                                <th scope="col" class="px-6 py-4 bg-gray-50">No.</th>
                                <th scope="col" class="px-6 py-4">Nama lengkap</th>
                                <th scope="col" class="px-6 py-4 bg-gray-50">Email</th>
                                <th scope="col" class="px-6 py-4">No. Telpon (Whatsapp)</th>
                                <th scope="col" class="px-6 py-4 bg-gray-50">Peran</th>
                        ...
                        @forelse ($accounts as $key => $account)
                            <tr>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ route('user.status', $account->id) }}" method="GET"class="inline-block">
                                        @csrf
                                        <button type="submit" class="{{ $account->status ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-red-500 hover:bg-red-600' }} px-3 py-2 rounded-xl text-white transition-all ease-in-out">
                                                    {!! $account->status ? '<i class="fa-solid fa-toggle-on"></i>' : '<i class="fa-solid fa-toggle-off"></i>' !!}
                                        </button>
                                    </form>
                                    <a href="{{ route('user.edit', $account->id) }}" class="inline-block bg-amber-500 hover:bg-amber-600 px-3 py-2 rounded-xl text-white transition-all ease-in-out">
                                        <i class="fa-regular fa-pen-to-square"></i>
                                    </a>
                                    <form action="{{ route('user.destroy', $account->id) }}" method="post" class="inline-block" onsubmit="return confirmDelete()">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="bg-red-500 hover:bg-red-600 px-3 py-2 rounded-xl text-white transition-all ease-in-out">
