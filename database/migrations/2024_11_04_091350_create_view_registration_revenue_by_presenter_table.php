...
class CreateViewRegistrationRevenueByPresenterTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        DB::statement('DROP VIEW IF EXISTS `registration_revenue_by_presenter`;');
        DB::statement('
        CREATE OR REPLACE VIEW `registration_revenue_by_presenter` AS
        SELECT
            status_applicants_registration.pmb AS pmb,
            users.identity AS identity_user,
            users.name AS name,
            SUM(status_applicants_registration.deal) AS total
        FROM
